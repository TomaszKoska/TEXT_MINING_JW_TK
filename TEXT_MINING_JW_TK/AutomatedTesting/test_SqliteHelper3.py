...
class Test_SqliteHelper3(unittest.TestCase):
    

    @classmethod
    def setUpClass(cls):
        try:
            os.remove(tempfile.gettempdir() + "/somedb.db")
        except:
            pass
        testHelper = SqliteHelper(tempfile.gettempdir() + "/somedb.db")
        testHelper.start()
        tableName = "docs"
        topicTableName = "tops"
    ...
    def setUp(self):
        self.testHelper = SqliteHelper(tempfile.gettempdir() + "/somedb.db")
